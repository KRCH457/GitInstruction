...
Для того, чтобы перейти на другую ветку необхххходимо использовать команду `git checkout "имя ветки"` ил `git switch "имя ветки"`.

 Чтобы создать ветку и сразу переключчччиться на неё, можно выполнить команду git checkout с параметром -b:
```
